...
class Main:
	...
	def banner(self):  # """ + self.generate_quote() + """
		...
		banner_fire_color = var.C_Green
		banner_middle_color = var.C_Red
		banner_bottom_color = var.C_Red
		banner_logo = ("""C_FIRE-----------------------------------------------------------C_W

C_FIRE    )       (         (              (      *        )  
C_FIRE ( /(  (    )\ )      )\ )     (     )\ ) (  `    ( /(  
C_FIRE )\()) )\  (()/(  (  (()/(     )\   (()/( )\))(   )\()) 
C_FIRE((_)((((_)( /(_)) )\  /(_)) ((((_)(  /(_)|(_)()\ ((_)\  
C_BOT _C_FIRE((_C_FIRE)\ _ C_FIRE)(_C_FIRE))_ C_FIRE((_C_FIRE)(_C_FIRE))    )\ C_BOT_ C_FIRE)\(_)) (_()((_)_ ((_) 
C_BOT| || C_FIRE(_)C_BOT_\C_FIRE(_C_FIRE)   C_BOT\| __/ __|   C_FIRE(_C_FIRE)C_BOT_\C_FIRE(_C_FIRE)C_BOT _ \|  \/  \ \ / / 
C_BOT| __ |/ _ \ | |) | _|\__ \    / _ \ |   /| |\/| |\ V /  
C_BOT|_||_/_/ \_\|___/|___|___/   /_/ \_\|_|_\|_|  |_| |_|   
                                                        
		~ GoblinCrack ~ 			
	     C_FIRE> C_BOTCurrent Powwer: 1TB C_FIRE<	 
C_FIRE------------------------<O>---------------------------------C_W""")
		...
		banner_logo = banner_logo.replace("C_BOT", banner_middle_color)
	...
	def _add_commands(self):
		...
		event.help(["exit", "quit", "e", "q"], "Exit -Hades.")
		event.help("help", "View all commands.")
		event.help("upgrade", "Upgrade Hades-Army.")
		...
		event.help("ddos", "Connect to a Hades-Army server.")
		event.help_comment("\nModules:")
	...
	def upgrade():
		try:
			chdir("/tmp")
			system("sudo git clone https://github.com/G0blinsCrackalacking/Hades-Army.git")
			chdir("/tmp/Hades-Army/")
			system("sudo bash ./install_to_bin.sh")
			print("[i] Deleting the temporary stored Hades-Army...")
			system("sudo rm -rf -i /tmp/Hades-Army/")
			var.stop()
			quit()
		except Exception:
			print("Cound not upgrade Hades-Army. (Make sure to use sudo)")
